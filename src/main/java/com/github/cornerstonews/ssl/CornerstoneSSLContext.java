...
public class CornerstoneSSLContext {
    ...
    public CornerstoneTrustStore getTrustStore() {
        try {
            Certificate certificate = new PEMFile(this.certificateFilePath).toCertificates().get(0);
            List<Certificate> caCertificates = new ArrayList<>();
            if (this.certificateChainFilePath != null) {
            ...
            int i = 0;
            for (Certificate caCertificate : caCertificates) {
                trustStore.setCertificateEntry("chain-cert-" + ++i, caCertificate);
            }
            ...
            File temp = File.createTempFile(".CornerstoneTrustStore", ".jts");
            temp.deleteOnExit();
            trustStore.store(new FileOutputStream(temp), randomPassword.toCharArray());
            
            return new CornerstoneTrustStore(temp.getAbsolutePath(), randomPassword);
        } catch (Exception e) {
            throw new RuntimeException("Cannot build keystore", e);
        }
    }
